...
class SentenceMatchModelGraph(object):
    def __init__(self, num_classes, word_vocab=None, char_vocab=None, POS_vocab=None,
                 NER_vocab=None,
                 ...
                 MP_dim=50, context_layer_num=1, aggregation_layer_num=1, fix_word_vec=False,
                 with_filter_layer=True, with_highway=False,
                 with_lex_features=False, lex_dim=100, word_level_MP_dim=-1, sep_endpoint=False,
                 end_model_combine=False, with_match_highway=False,
                 with_aggregation_highway=False, highway_layer_num=1, with_lex_decomposition=False,
        ...
        if with_word and word_vocab is not None:
            ...
            self.in_passage_words = tf.placeholder(tf.int32,
                                                   [None, None])  # [batch_size, passage_len]
            #             self.word_embedding = tf.get_variable("word_embedding",
            # shape=[word_vocab.size()+1, word_vocab.word_dim], initializer=tf.constant(
            # word_vocab.word_vecs), dtype=tf.float32)
        ...
        if with_POS and POS_vocab is not None:
            self.in_question_POSs = tf.placeholder(tf.int32,
                                                   [None, None])  # [batch_size, question_len]
            self.in_passage_POSs = tf.placeholder(tf.int32,
                                                  [None, None])  # [batch_size, passage_len]
            #             self.POS_embedding = tf.get_variable("POS_embedding",
            # shape=[POS_vocab.size()+1, POS_vocab.word_dim], initializer=tf.constant(
            # POS_vocab.word_vecs), dtype=tf.float32)
            self.POS_embedding = tf.get_variable("POS_embedding",
                                                 initializer=tf.constant(POS_vocab.word_vecs),
                                                 dtype=tf.float32)

            in_question_POS_repres = tf.nn.embedding_lookup(self.POS_embedding,
                                                            self.in_question_POSs)  # [
            # batch_size, question_len, POS_dim]
            in_passage_POS_repres = tf.nn.embedding_lookup(self.POS_embedding,
                                                           self.in_passage_POSs)  # [batch_size,
            # passage_len, POS_dim]
        ...
        if with_NER and NER_vocab is not None:
            self.in_question_NERs = tf.placeholder(tf.int32,
                                                   [None, None])  # [batch_size, question_len]
            self.in_passage_NERs = tf.placeholder(tf.int32,
                                                  [None, None])  # [batch_size, passage_len]
            #             self.NER_embedding = tf.get_variable("NER_embedding",
            # shape=[NER_vocab.size()+1, NER_vocab.word_dim], initializer=tf.constant(
            # NER_vocab.word_vecs), dtype=tf.float32)
            self.NER_embedding = tf.get_variable("NER_embedding",
                                                 initializer=tf.constant(NER_vocab.word_vecs),
                                                 dtype=tf.float32)

            in_question_NER_repres = tf.nn.embedding_lookup(self.NER_embedding,
                                                            self.in_question_NERs)  # [
            # batch_size, question_len, NER_dim]
            in_passage_NER_repres = tf.nn.embedding_lookup(self.NER_embedding,
                                                           self.in_passage_NERs)  # [batch_size,
            # passage_len, NER_dim]
        ...
        if with_char and char_vocab is not None:
            ...
            # shape=[char_vocab.size()+1, char_vocab.word_dim], initializer=tf.constant(
            # char_vocab.word_vecs), dtype=tf.float32)
            self.char_embedding = tf.get_variable("char_embedding",
            ...
            in_passage_char_repres = tf.reshape(in_passage_char_repres,
                                                shape=[-1, p_char_len, char_dim])
            passage_char_lengths = tf.reshape(self.passage_char_lengths, [-1])
            with tf.variable_scope('char_lstm'):
                # lstm cell
                char_lstm_cell = contrib.rnn.BasicLSTMCell(char_lstm_dim)
                # dropout
                if is_training: char_lstm_cell = contrib.rnn.DropoutWrapper(char_lstm_cell,
                                                                            output_keep_prob=(
                                                                                1 - dropout_rate))
                char_lstm_cell = contrib.rnn.MultiRNNCell([char_lstm_cell])
        ...
        # ========Bilateral Matching=====
        (match_representation, match_dim) = match_utils.bilateral_match_func2(in_question_repres,
                                                                              in_passage_repres,
        ...
        # tf.cast(self.truth, tf.int64), name='cross_entropy_per_example')
        ...
        if optimize_type == 'adadelta':
            ...
            self.global_step = tf.Variable(0, name='global_step',
                                           trainable=False)  # Create a variable to track the
            # global step.
            min_lr = 0.000001
            self._lr_rate = tf.maximum(min_lr,
                                       tf.train.exponential_decay(learning_rate, self.global_step,
                                                                  30000, 0.98))
            self.train_op = tf.train.GradientDescentOptimizer(learning_rate=self._lr_rate).minimize(
                self.loss)
    ...
    in_question_words = property(get_in_question_words, set_in_question_words,
                                 del_in_question_words, "in_question_words's docstring")
    in_passage_words = property(get_in_passage_words, set_in_passage_words, del_in_passage_words,
                                "in_passage_words's docstring")
    word_embedding = property(get_word_embedding, set_word_embedding, del_word_embedding,
                              "word_embedding's docstring")
    ...
    POS_embedding = property(get_pos_embedding, set_pos_embedding, del_pos_embedding,
                             "POS_embedding's docstring")
    ...
    NER_embedding = property(get_ner_embedding, set_ner_embedding, del_ner_embedding,
                             "NER_embedding's docstring")
    question_char_lengths = property(get_question_char_lengths, set_question_char_lengths,
                                     del_question_char_lengths, "question_char_lengths's docstring")
    passage_char_lengths = property(get_passage_char_lengths, set_passage_char_lengths,
                                    del_passage_char_lengths, "passage_char_lengths's docstring")
    in_question_chars = property(get_in_question_chars, set_in_question_chars,
                                 del_in_question_chars, "in_question_chars's docstring")
    in_passage_chars = property(get_in_passage_chars, set_in_passage_chars, del_in_passage_chars,
                                "in_passage_chars's docstring")
    char_embedding = property(get_char_embedding, set_char_embedding, del_char_embedding,
                              "char_embedding's docstring")
    ...
    global_step = property(get_global_step, set_global_step, del_global_step,
                           "global_step's docstring")
    lr_rate = property(get_lr_rate, set_lr_rate, del_lr_rate, "lr_rate's docstring")
    eval_correct = property(get_eval_correct, set_eval_correct, del_eval_correct,
                            "eval_correct's docstring")
    predictions = property(get_predictions, set_predictions, del_predictions,
                           "predictions's docstring")
