...
class Vocab(object):
    ...
    def fromVocabualry(self, voc, dim=100):
        ...
        scale = 0.05
        self.word_vecs = np.array(np.random.uniform(low=-scale, high=scale, size=shape),
                                  dtype=np.float32)
    ...
    def fromBinary_with_voc(self, fname, voc, scale=0.05, stop_num=50):
        ...
        self.vocab_size = len(self.word2id)
        self.word_vecs = np.random.uniform(low=-scale, high=scale,
                                           size=(self.vocab_size + 1, self.word_dim)).astype(
            'float32')
    ...
    def fromBinary(self, fname, scale=0.05, voc=None):
        ...
        if voc is not None:
            for word in voc:
                ...
                self.word2id[word] = curIndex
                self.id2word[curIndex] = word
                word_vecs[curIndex] = np.random.uniform(low=-scale, high=scale,
                                                        size=(self.word_dim,)).astype('float32')
