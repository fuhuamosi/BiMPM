...
def _dynamic_rnn_loop(cell, inputs, initial_state, parallel_iterations, swap_memory,
    ...
    def _create_ta(name, dtype):
        return tensor_array_ops.TensorArray(dtype=dtype, size=time_steps,
                                            tensor_array_name=base_name + name,
                                            clear_after_read=False)

    output_ta = tuple(_create_ta("output_%d" % i, rnn._infer_state_dtype(dtype, state)) for i in
                      range(len(flat_output_size)))
    input_ta = tuple(
    ...
    def _time_step(time, output_ta_t, state):
        ...
        def f2():
            return tuple(ta.read(tf.subtract(time, 1)) for ta in output_ta_t)  # output_ta_t

        # .read(tf.subtract(time, 1))
